...
const compress =
  (src, name, dest = "./build") =>
  () => {
    gulp.src(src).pipe(zip(name)).pipe(gulp.dest(dest));
  };

gulp.task("compress:app", compress("./build/app/**", "app.zip"));
...
gulp.task(
  "compress:win32-ia32",
  compress(
    `./build/executables/${config.productName}-win32-ia32/**`,
    "win32-ia32.zip"
  )
);
gulp.task(
  "compress:win32-x64",
  compress(
    `./build/executables/${config.productName}-win32-x64/**`,
    "win32-x64.zip"
  )
);
gulp.task(
  "compress:linux-ia32",
  compress(
    `./build/executables/${config.productName}-linux-ia32/**`,
    "linux-ia32.zip"
  )
);
gulp.task(
  "compress:linux-x64",
  compress(
    `./build/executables/${config.productName}-linux-x64/**`,
    "linux-x64.zip"
  )
);
